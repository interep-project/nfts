...
task("deploy", "Deploy an InterepNFT contract")
    ...
    .setAction(async ({ logs, interepAddress }, { ethers }): Promise<Contract> => {
        const ContractFactory = await ethers.getContractFactory("InterepNFT")

        const contract = await ContractFactory.deploy(interepAddress)

        await contract.deployed()

        logs && console.log(`InterepNFT contract has been deployed to: ${contract.address}`)
